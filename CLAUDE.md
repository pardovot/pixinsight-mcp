...
2. **PixInsight update repo** (`pi-repo/`): users add one URL; PixInsight auto-installs the **native module** (`type="module"`, into `bin/`) straight from Resources > Updates, no source build. Rebuild with `npm run repo:build` (packages `module/build/MCPWatcher-pxm.*` per platform, reproducible pure-Node zip, generates `updates.xri`). **`updates.xri` is signed by `npm run repo:sign`; see the note below.** (Pre-2026-07-24 the flow ended with `node module/sign.mjs pi-repo/updates.xri`, native `--sign-xml-file`, ~5 s.) Repackaged from the JS watcher to the module 2026-07-22.
