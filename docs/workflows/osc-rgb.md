...
- **Star COLOR correction, gated + measured [R8, user technique]:** after the star stretch, measure the star-pixel hue and apply **only what fires**, (a) **green-dominant** stars → `SCNR` green (Average Neutral); (b) **magenta / purple / red-fringed** stars → **`invert → SCNR green → invert`** (magenta is green's complement, so this removes the purple fringe and shifts those stars toward yellow). Same discipline as the nebula SCNR gate, measure the star pixels first, never apply both blindly. (Broadband star fields especially benefit: SPCC gives good bulk star color but leaves purple fringing on bright/saturated stars.)
